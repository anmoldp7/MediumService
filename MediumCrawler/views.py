...
def save_tag(req_tag):
    current_tag = AccessedTag(tag_name = req_tag)
    current_tag.save()
    url = "https://medium.com/tag/" + req_tag
...
def home_page(request):
    ...
    if request.method == "POST":
        ...
        if submitted_tag.is_valid():
            save_tag(request.POST["tag_name"])
            return redirect("tag/" + request.POST["tag_name"])
        else:
...
def tag_page(request, pk):
    ...
    except AccessedTag.DoesNotExist:
        return redirect("home_page")
